...
function LUI:ApplyUpdate(ver)
		...
		if unitframes and unitframes.db then
			...
			for _, unit in ipairs(unitframes.units) do
				...
				if unitDB then
					...
					if unitDB.NameText then unitDB.NameText.Length = nil end
					if unitDB.Aura then
	...
	if ver < 9 then
		...
		if artwork and artwork.db then
			local artDB = artwork.db.profile
			local customPanels = rawget(artDB, "Textures")
			if customPanels then
				for panelName in pairs(customPanels) do
					artDB.Colors[panelName] = nil
				end
				artDB.Textures = nil
			end
		end
