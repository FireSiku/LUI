...
function module:CreateXpRepOptionsPart(barType, order)
	...
	if barType == "XP" then
		Toggle = function()
			if not oUF_LUI_player.Experience then oUF_LUI.funcs.Experience(oUF_LUI_player, oUF_LUI_player.__unit, xprepdb) end
		...
		Toggle = function()
			if not oUF_LUI_player.Reputation then oUF_LUI.funcs.Reputation(oUF_LUI_player, oUF_LUI_player.__unit, xprepdb) end
	...
	local ApplySettings = function()
		if oUF_LUI_player.Experience then oUF_LUI.funcs.Experience(oUF_LUI_player, oUF_LUI_player.__unit, xprepdb) end
		if oUF_LUI_player.Reputation then oUF_LUI.funcs.Reputation(oUF_LUI_player, oUF_LUI_player.__unit, xprepdb) end
	end
...
function module:CreateBarOptions(barType, order)
	...
	local ToggleFunc = function(self, Enable)
		if not oUF_LUI_player[barKey] then oUF_LUI.funcs[barKey](oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player) end
		if Enable then
	...
	local ApplySettings = function()
		oUF_LUI.funcs[barKey](oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player)
		oUF_LUI_player:UpdateAllElements()
	end
	...
	local options = {
		...
		args = {
			...
			Colors = (not isLockable) and {
				...
				order = 3,
				args = {
					ColorType = LUI:NewSelect("Color", "Choose the Color Option for "..barName..".", 1, {"By Class", "Individual"}, nil, bardb, "Color", bardefaults, ApplySettings),
...
function module:LoadOptions()
	local ToggleDruidManaBar = function(self, Enable)
		if not oUF_LUI_player.DruidMana then oUF_LUI.funcs.DruidMana(oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player) end
		if Enable then
	...
	local StyleDruidMana = function()
		...
		oUF_LUI.funcs.DruidMana(oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player)
	...
	local StylePvP = function()
		oUF_LUI.funcs.PvP(oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player)
		oUF_LUI_player:UpdateAllElements()
	end
	
	local ToggleAltPower = function(self, Enable)
		if not oUF_LUI_player.AltPowerBar then
			oUF_LUI.funcs.AltPowerBar(oUF_LUI_player, oUF_LUI_player.__unit, db.oUF.Player)
			if oUF_LUI_pet then oUF_LUI.funcs.AltPowerBar(oUF_LUI_pet, oUF_LUI_pet.__unit, db.oUF.Pet) end
		end
